...
valid_version "$version"; valid_revision "$revision"; valid_bool "$with_userspace"; valid_bool "$with_acm"; valid_bool "$accept_deptp_eula"; valid_interface "$edge_interface"
mkdir -p "$out_dir"
...
if [[ "$with_userspace" == true ]]; then
  "$SCRIPT_DIR/build-userspace.sh" --source "$source_dir" --version "$version" --revision "$revision" --out "$out_dir" --maintainer "$maintainer" --with-acm "$with_acm" --accept-deptp-eula "$accept_deptp_eula"
fi
